...
| Return observation | `payment/context.tsx`                     | Visible focus/visibility performs at most three coalesced result GETs with bounded delays; explicit check makes one GET; neither opens payment | observer and controller tests          |
...
The payment panel reuses the existing rounded white card, CKS-red primary action, native button semantics, focus ring, and narrow-shell behavior. It does not display checkout URLs, quote tokens, PaymentIntent IDs, raw backend/provider codes, or provider redirect parameters. Backend-confirmed FAILED and authoritative create rejections release the payment freeze while retaining basket lines and require a fresh quote. A changed payable total requires explicit acceptance. Uncertain initiation reuses its original idempotency key; an unresolved intent cannot restart. Leaving a verified paid order clears the completed basket.
